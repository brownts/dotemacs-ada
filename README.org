...
- User Option: init.el/lsp-mode-specific-settings ::
  Settings for ~lsp-mode~ which are to be applied to specific major modes are captured here.  This may enable or disable certain features which are desired only in a subset of modes and thus differ from the default configuration.  This is an attribute list where the key is the major mode and the value is a property list.  The property list key is the variable name to be set buffer-local and the value is the desired value for the variable.  The variables are expected to be ~lsp-mode~ customization variables (e.g., ~lsp-semantic-tokens-enable~).
